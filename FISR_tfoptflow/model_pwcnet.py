...
from tensorflow_addons.layers import CorrelationCost as cost_volume
...
class ModelPWCNet(ModelBase):
    ...
    def setup_metrics_ops(self):
        ...
        if self.opts['sparse_gt_flow'] is True:
            ...
            # Zero out flow predictions at the same location so we only compute the EPE at the sparse flow points
            flow_pred_tnsr = tf.where(zeros_loc, tf.zeros_like(flow_pred_tnsr), flow_pred_tnsr)
...
def pwcnet_loss(y_true, y_pred):
    # Simple L2 Loss between ground truth and predicted optical flow
